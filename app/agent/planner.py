...
class Planner:
    ...
    def _generate_qwen_plan(self, question, intent) -> str:
        prompt = f"""
You are the planning component of a private,
on-premise industrial AI agent.

Create a short execution plan for the user's request.

The plan will be shown in the frontend as agent progress.

Do NOT reveal hidden chain-of-thought.
Do NOT provide the final answer.
Only describe the high-level actions the agent will perform.
...
- If intent is "rag", the plan MUST contain:
  1. rag_search
  2. generate_response
...
{{
    ...
    "steps": [
        {{
            "id": 1,
            "tool": "rag_search",
            "action": "search_local_knowledge",
            "params": {{
                "query": "useful retrieval query"
            }},
            "description": "Search the local knowledge base for relevant evidence."
        }},
        {{
            "id": 2,
            "tool": "generate_response",
            "action": "synthesize_grounded_answer",
            "params": {{}},
            "description": "Generate the final answer from verified evidence."
        }}
    ],
    "final_output": "grounded_answer"
}}
